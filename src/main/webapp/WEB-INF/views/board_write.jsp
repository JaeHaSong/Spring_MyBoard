...
<script>
	$(function()
	{
	    ...
	    function sendFile(file, editor)
	    {
		    ...
		    {
			    $(editor).summernote('editor.insertImage', "/resources/${loginId}/"+data);
...
<body>
	...
	<div class="container">
		...
		<input id="regNotice" class="btn btn-info" type="button" value="작성 완료">

	</div>
	
</body>
<c:if test="${ rewriting == null }">
...
<script>
	...
	$(function()
	{
	    ...
	    {
	    	var form = $('<form></form>');
	    	form.attr('action', 'board_write_do');
...
<script>
	...
	$(function()
	{
	    ...
	    {
	    	var form = $('<form></form>');
	    	form.attr('action', 'board_update_do');
	    	form.attr('method', 'POST');
	    	form.appendTo('body');
		    ...
		    if((text != "<br>") && ($("#title_text").val() != ""))
		    {
		    	var seq = $('<input type="hidden" value="${seq}" name="b_seq">');
		    	var writer = $('<input type="hidden" value='+"${ loginId }"+' name="b_writer">');
		    	var title = $('<input type="hidden" name="b_title">');
		    	var contents = $('<input type="hidden" id="contents_hidden" name="b_contents">');
		    	
		   		form.append(seq).append(writer).append(title).append(contents);
		    	$("#contents_hidden").val(text);
		    	title.val($("#title_text").val());
